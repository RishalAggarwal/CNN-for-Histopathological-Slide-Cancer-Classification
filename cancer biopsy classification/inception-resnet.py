...
model=InceptionResNetV2(include_top=False,
                      ...
                      pooling='max',
                      dropout_keep_prob=0.8)
outputs=outputs = Dense(2, activation='softmax')(model.output)
model = Model(model.inputs, outputs)
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
checkpt = ModelCheckpoint('inception_resnet_models/40X/weights.{epoch:02d}.hdf5', monitor='val_acc', verbose=1,
                              save_best_only=True)
'''model.fit_generator(
        train_generator,
        steps_per_epoch=297,
